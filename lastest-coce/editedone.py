...
Header = [
    "Account ID", "Account Name", "Region", "Record Name", "Record Type", "Value/Route Traffic To",
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Initialize region_data dictionary
        region_data = {}
        ...
        for account in accounts:
            ...
            if account_id != LAMBDA_ACCOUNT_ID:
                ...
                process_account(account_id, account_name, lambda_security_audit_role, region_data)
        ...
        # Step 7: Write consolidated CSV files for each region
        for region, data in region_data.items():
            ...
            consolidated_file_name = f"consolidated_{region}.csv"
            upload_to_s3(csv_buffer, S3_BUCKET_NAME, consolidated_file_name)
            Logger.info(f"Uploaded consolidated CSV for region {region}")
    ...
    return {
        'statusCode': 200,
        'body': json.dumps(f"Consolidated CSV files uploaded successfully to S3 bucket {S3_BUCKET_NAME}")
    }
...
def process_account(account_id, account_name, security_audit_role, region_data):
    ...
    try:
        ...
        for region in regions:
            ...
            try:
                ...
                process_records(account_id, account_name, region, route53, elbv2, elb_dns_map, region_data)
            except ClientError as e:
...
def process_records(account_id, account_name, region, route53, elbv2, elb_dns_map, region_data):
    ...
    for page in paginator.paginate():
        for zone in page['HostedZones']:
            ...
            for record in records['ResourceRecordSets']:
                row = process_record(account_id, account_name, region, record, zone_id, zone_type, elbv2, elb_dns_map)
                if region not in region_data:
                    region_data[region] = []
                region_data[region].append(row)

def process_record(account_id, account_name, region, record, zone_id, zone_type, elbv2, elb_dns_map):
    ...
    return [
        account_id, account_name, region, record_name, record_type, value, alias, ttl, routing_policy,
