...
def main():
    try:
        # Step 1: Get DNS records for delegated account
        Logger.info(f"Assuming SecurityAuditRole in Lambda account ({LAMBDA_ACCOUNT_ID})")
        lambda_role = assume_role(LAMBDA_ACCOUNT_ID, 'SecurityAuditRole')
        if not lambda_role:
            raise Exception('Failed to assume SecurityAuditRole in Lambda account')

        lambda_csv_buffer = io.StringIO()
        lambda_writer = csv.writer(lambda_csv_buffer)
        lambda_writer.writerow(Header)
        process_account(lambda_writer, LAMBDA_ACCOUNT_ID, lambda_role)
        ...
        management_role = assume_role(MANAGEMENT_ACCOUNT_ID, 'SecurityAuditRole', lambda_role)
        if not management_role:
            raise Exception('Failed to assume role in management account')
        ...
        for account in accounts:
            ...
            if account_id != LAMBDA_ACCOUNT_ID and account_id != MANAGEMENT_ACCOUNT_ID:
                process_account(writer, account_id, lambda_role)
        ...
        Logger.info(f"CSV files uploaded successfully to S3 bucket {S3_BUCKET_NAME}")
...
def assume_role(account_id, role_name='SecurityAuditRole', source_credentials=None):
    ...
    # Retrieve current role ARN
    current_role_arn = None
    if source_credentials:
        try:
            current_identity = sts.get_caller_identity()
            current_role_arn = current_identity.get('Arn', '')
        except ClientError as e:
            Logger.error(f"Failed to get current role ARN: {str(e)}")

    # Check if the role is the same as the current role
    if current_role_arn and current_role_arn == role_arn:
        Logger.error(f"Attempted to assume the same role: {role_arn}")
        return None
...
def process_listener(writer, account_id, region, elb, listener, elbv2, dns_map):
    ...
    for tg_arn in tg_arns:
        ...
        # Write the DNS information for the listener
        writer.writerow([
            account_id, region, elb['DNSName'], 'A', '60', 'Public', 
            elb['LoadBalancerArn'], listener['ListenerArn'], tg_name, 
            'N/A', 'N/A', 'N/A', 'N/A', 'N/A'
        ])

def process_non_elb_records(writer, account_id, region, route53):
    dns_map = get_dns_map(route53)
    for record_name, record_info in dns_map.items():
        writer.writerow([
            account_id, region, record_name, record_info['Type'], 
            record_info['TTL'], record_info['ZoneType'], record_info['ZoneId'], 
...
def upload_account_list_to_s3(accounts, bucket_name, file_name):
    try:
        ...
        with io.StringIO() as buffer:
            json.dump(accounts, buffer, indent=4)
            buffer.seek(0)
            s3.upload_fileobj(buffer, bucket_name, file_name)
            Logger.info(f"Successfully uploaded account list to S3 bucket {bucket_name}")
    except ClientError as e:
