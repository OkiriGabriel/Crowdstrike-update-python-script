...
Header = [
    ...
    "Hosted Zone ID", "Load Balancer ARN", "Load Balancer DNS Name", "Listener ARN", 
...
def process_record(writer, account_id, region, record, zone_id, zone_type, elbv2, elb_dns_map):
    ...
    # Check if this record points to an ELB
    associated_elb = None
    for elb_dns, elb in elb_dns_map.items():
        if elb_dns in record_name or (record.get('AliasTarget') and elb_dns in record['AliasTarget'].get('DNSName', '')):
            associated_elb = elb
            break
    ...
    if associated_elb:
        ...
        for listener in listeners:
            ...
            if tg_arn:
                ...
                tg_name = health_check_enabled = health_check_protocol = health_check_port = health_check_path = 'N/A'

            writer.writerow([
                account_id, region, record_name, record_type, ttl, zone_type, zone_id,
                lb_arn, lb_dns_name, listener_arn, tg_name, tls_enabled,
                health_check_enabled, health_check_protocol, health_check_port, health_check_path
            ])
    else:
        # If no associated ELB, fill in N/A for ELB-related fields
        writer.writerow([
            account_id, region, record_name, record_type, ttl, zone_type, zone_id,
